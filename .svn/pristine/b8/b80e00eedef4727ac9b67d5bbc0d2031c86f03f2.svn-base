<%@ WebService Language="C#" Class="userServices" %>
    ...
    public string getUserInfo(string contextKey)
    {
        ...
        SqlConnection oConn = new SqlConnection(oConnString);
            ...
            while (oReader.Read())
            {
                ...
                Retval = string.Format("{0}<tr><td><a href='../Profile.aspx?ID={2}' title='view profile' target='_blank'><img src='EmpImage.aspx?EMPID={2}&W=60&H=60&imgid={3}' width='60'></a></td><td nowrap=nowrap>{1}</td></tr>", Retval, oReader["Emp"], contextKey, oReader["ImageIdentifier"]);
