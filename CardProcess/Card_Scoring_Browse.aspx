<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true"
    CodeFile="Card_Scoring_Browse.aspx.cs" Inherits="Card_Scoring_Browse" %>
        ...
        <ContentTemplate>
            <div class="Panel1 ui-corner-all" style="display: inline-block; line-height: 2.5em">
                 ...
                 <div class="form-inline">
                     App ID:
                    <asp:TextBox ID="txtID" runat="server" Width="50px" onfocus="this.select()"
                        Watermark="app id" ></asp:TextBox>
                </div>
                <div class="form-inline">
                    Filter:
                ...
                <div class="form-inline">
                    <asp:Button ID="cmdFilter" runat="server" Font-Bold="true" Text="Show" Width="80px" OnClick="cmdFilter_Click" />
                </div>

            </div>
            <br />
            <asp:GridView ID="GridView1" runat="server" CssClass="Grid" AutoGenerateColumns="False" PageSize="20"
                DataKeyNames="SL" DataSourceID="SqlDataSource1" BackColor="White" BorderColor="#DEDFDE"
                ...
                AllowSorting="True"  EmptyDataText="Data Not Found">
                <PagerSettings Position="TopAndBottom" PageButtonCount="30" />
                <RowStyle BackColor="#F7F7DE" VerticalAlign="Top" />
                <Columns>
                    <asp:TemplateField HeaderText="#" SortExpression="SL">
                        <ItemTemplate>
                            <a class="HoverLink" title="Open" target="_blank" href='<%# "Card_Scoring.aspx?id=" + Eval("SL") %>'>
                                <%# Eval("SL")%></a>
                        </ItemTemplate>
                    ...
                    <asp:TemplateField HeaderText="Applicant" SortExpression="Applicant">
                        <ItemTemplate>
                            <%# Eval("Applicant") %>
                        </ItemTemplate>
                    </asp:TemplateField>
                    ...
                    <asp:TemplateField HeaderText="Update By" InsertVisible="False" SortExpression="Updated_By">
                        <ItemTemplate>
                            <uc3:EMP ID="EMP2" Username='<%# Eval("Updated_By") %>' runat="server" />
                            <span class="time-small" title='<%# Eval("Update_Date_Time", "{0:dddd, dd MMMM yyyy, hh:mm:ss tt}")%>'>
                                <time class="timeago" datetime='<%# Eval("Update_Date_Time","{0:yyyy-MM-dd HH:mm:ss}") %>'></time>
                            </span>
                        </ItemTemplate>
                    </asp:TemplateField>
                    ...
                    <asp:TemplateField HeaderText="Verified By" InsertVisible="False" SortExpression="Verified_By">
                        <ItemTemplate>
                            <uc3:EMP ID="EMP4" Username='<%# Eval("Verified_By") %>' runat="server" />
                            <span class="time-small" title='<%# Eval("Verified_DT", "{0:dddd, dd MMMM yyyy, hh:mm:ss tt}")%>'>
                                <time class="timeago" datetime='<%# Eval("Verified_DT","{0:yyyy-MM-dd HH:mm:ss}") %>'></time>
                            </span>
                        </ItemTemplate>
                    </asp:TemplateField>
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:CardDataConnectionString %>"
                SelectCommand="usp_Credit_Browse" SelectCommandType="StoredProcedure" OnSelected="SqlDataSource1_Selected">
                <SelectParameters>
                    <asp:ControlParameter Name="ID" ControlID="txtID" PropertyName="Text" Type="Int32"
                        DefaultValue="-1" />
