...
public partial class ITCL: System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Response.Clear();
            Response.Write("<h1>You have no permisssion to view this page.</h1>");
            Response.End();
        }
        else
        {
        ...
        if (!IsPostBack)
        {
            ...
            hypTravelDetails.NavigateUrl = string.Format("VIP_Tours.aspx?ITCLID={0}", ID);
        }
    ...
    {
        Response.Redirect("VIP_Tours.aspx?ITCLID="+HidITCLID.Value.ToString(),true);  
    }
    ...
    public string getLinkImage1(object AID, object FileKey, object Extension)
    {
        string RetVal = "";
        RetVal = "ShowImage.ashx?aid=" + AID.ToString() + "&key=" + FileKey.ToString() + "&w=400&h=400&z=1&p=1&type=itcl";
        return RetVal;
    }
    ...
    protected void LinkOpen_Click(object sender, EventArgs e)
    {
        try {
            ...
            hidAttachmentID.Value = AttachmentID;
            Literal1.Text = "Attachment Details #" + AttachmentID.ToString();
            Modal2.Show();
        }
        catch(Exception exx)
    ...
    protected void SqlDataSource1_Selected(object sender, SqlDataSourceStatusEventArgs e)
    {
        if (e.AffectedRows > 0)
        {
            //Literal1.Text = "Attachment Details#" +"420";
        }            
    }

    protected void SqlDataSource33_Selected(object sender, SqlDataSourceStatusEventArgs e)
    {
        if (!IsPostBack)
        {
        ...
        if(e.AffectedRows > 0)
        {
            bool ManuallySaved = (bool)e.Command.Parameters["@ManuallySaved"].Value;
            //TrustControl1.ClientMsg(ManuallySaved.ToString());
            if (ManuallySaved == false)
            {
                //Found in Old Cards
                //DetailsView2.Visible = false;
                cmdNew.Visible = false;
                hypTravelDetails.Visible = false;
            }
            else
            {
                //Found in ITCL Customer Table
                hypTravelDetails.Visible = true;
            }
        }
        else
        {
    ...
    {
        Response.Redirect("ITCL.aspx?ITCLID=" + Request.QueryString["ITCLID"].ToString(), true);
    }
    ...
    protected void DetailsView2_DataBound(object sender, EventArgs e)
    {
        ...
        //{           
        //    for (int i = 1; i < DetailsView2.Rows.Count - 3; i++)
        //    {
        //        if (DetailsView2.Rows[i].Cells.Count == 2)
        //        {
        //            string value = DetailsView2.Rows[i].Cells[1].Text.Trim().Replace("&nbsp;", "");


        //            if (DetailsView2.Rows[i].Cells[1].HasControls())
        //            {
        //                try
        //                {
        //                    value = ((DataBoundLiteralControl)DetailsView2.Rows[i].Cells[1].Controls[0]).Text.Trim().Replace("&nbsp;", "");
        //                }
        //                catch (Exception) { }
        //            }
        ...
        //            {
        //                DetailsView2.Rows[i].Visible = false;
        //            }
        //        }
        //    }
        //}
    ...
    protected void SqlDataSource33_Updated(object sender, SqlDataSourceStatusEventArgs e)
    {
        string msg = e.Command.Parameters["@Msg"].Value.ToString();
        TrustControl1.ClientMsg(msg);
    }
