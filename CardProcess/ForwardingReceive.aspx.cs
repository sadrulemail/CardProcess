...
{   
    ...
    {
        Response.Redirect("ForwardingBranchView.aspx?id="+txtFilter.Text.ToString(),true);
    ...
    protected void downloadxlsx_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                ExcelWorksheet worksheet = xlPackage.Workbook.Worksheets.Add("Card Received");
                int StartRow = 1;
                ...
                worksheet.Cells[StartRow, 5].Value = "Request Type";
                worksheet.Cells[StartRow, 6].Value = "Delivery Branch";
                ...
                for (int r = 0; r < DV.Table.Rows.Count; r++)
                {
                    ...
                    if (DV.Table.Rows[r]["ReqTypeName"] != DBNull.Value)
                        worksheet.Cells[R, 5].Value = DV.Table.Rows[r]["ReqTypeName"].ToString();
                    if (DV.Table.Rows[r]["BranchName"] != DBNull.Value)
                        worksheet.Cells[R, 6].Value = DV.Table.Rows[r]["BranchName"].ToString();
                ...
                //Adding Properties
                xlPackage.Workbook.Properties.Title = "Card Received" + DateTime.Now.Date.ToString("dd/MM/yyyy");
            ...
            Response.AddHeader("Content-Disposition", "attachment;filename=" + "Card_Received_" + DateTime.Now.Date.ToString("dd/MM/yyyy") + ".xlsx");
