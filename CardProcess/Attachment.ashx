...
public class Attachment : IHttpHandler, IReadOnlySessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (DR == null)
        {
            ...
            {
                string Query = "";
                //Query = string.Format(string.Format("EXEC s_Attachments_Download '{0}', '{1}', '{2}', '{3}', {4}"
                //        , AID, FileID, EMPID, getIPAddress(), Type
                //    ));
                Query = "s_Attachments_Download";
                ...
                using (SqlCommand cmd = new SqlCommand(Query, conn))
                {
                    ...
                    cmd.Parameters.Add("@IP", SqlDbType.VarChar).Value = getIPAddress();
