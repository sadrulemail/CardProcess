...
public partial class Supplementary_Export : System.Web.UI.Page
{
        ...
        if (!IsPostBack && !string.IsNullOrEmpty(Request.QueryString["batch"]) && !string.IsNullOrEmpty(Request.QueryString["type"]))
        {
            ...
            {                
                ...
                try
                {
                    ...
                    {
                        ExcelWorksheet worksheet = xlPackage.Workbook.Worksheets.Add("SupplementaryCard");
                        int StartRow = 1;
    ...
    protected void GridView6_RowCommand(object sender, EventArgs e)
    {
        ...
        try
        {
            foreach (GridViewRow row in GridView6.Rows)
            {
                ...
                {
                    CheckBox chkRow = (row.Cells[0].FindControl("rowLevelCheckBox") as CheckBox);
                    if (chkRow.Checked)
                    {
                        TotalCards += checkValue(row.Cells[4].Text);
                    }
                }
            }
            GridView6.FooterRow.Cells[3].Text = "Total Card(s)";
            GridView6.FooterRow.Cells[4].Text = TotalCards.ToString();
        }
        catch (Exception exx)
        {
                ...
                if (IDs.Length >= 2)
                {
                    ...
                    using (SqlConnection conn = new SqlConnection())
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand())
                        {
                            ...
                            lblDownloadLink.Text = string.Format("<a href='Supplementary_Export_Download.aspx?batch={0}&type=xml' class='Link2'>Download Batch # {0}<a>", Sql_Msg.Value);
                        }
                    }
    ...
    protected void SqlDataSourceSearch_Selected(object sender, SqlDataSourceStatusEventArgs e)
    {
        lblmsg1.Visible = e.AffectedRows > 0;
        lblmsg1.Text = "Total Record(s):<b>"+e.AffectedRows.ToString()+"</b>";
    }
