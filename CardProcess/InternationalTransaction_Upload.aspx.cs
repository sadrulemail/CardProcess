        ...
        foreach (string line in lines)
        {
            ...
            if (cols.Length > 10)
            {
                if (int.TryParse(cols[1].Trim().ToString(), out dt))
                {
                    ...
                    if (decimal.TryParse(colss[31].Trim(), out d))
                        TransactionAmount = decimal.Parse(colss[31].Trim());
                    if (decimal.TryParse((colss[32].Trim().ToString()).Replace("=", "").Trim(), out d))
                        EffectedRemittance = decimal.Parse((colss[32].Trim().ToString()).Replace("=", "").Trim());
                    if (decimal.TryParse(colss[33].Trim().Replace(",", ""), out d))
                        CardLimit = decimal.Parse(colss[33].Trim().Replace(",", ""));
                    if (decimal.TryParse(colss[34].Trim().Replace(",", ""), out d))
                        FCLimitNONSAARC = decimal.Parse(colss[34].Trim().Replace(",", ""));
                    if (decimal.TryParse(colss[35].Trim().Replace(",", ""), out d))
                        FCLimitSAARC = decimal.Parse(colss[35].Trim().Replace(",", ""));
                    ...
                    dt_InternationalTransaction.Rows.Add(txtYMID.Text.Trim(), HidPageID.Value, colss[1].Trim(),
                        ...
                        CardLimit, FCLimitNONSAARC, FCLimitSAARC, colss[36].Trim().Replace(",", ""));
                }
            }
        }
        DataRow[] rows = dt_InternationalTransaction.Select("IssueDate < '01/01/1753'");// AND date <= #" + to_date + "#");

        DataRow[] rowss = dt_InternationalTransaction.Select("ClientID='2812064'");
